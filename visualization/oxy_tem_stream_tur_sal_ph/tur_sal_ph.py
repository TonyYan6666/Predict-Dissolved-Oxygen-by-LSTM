...
for idx, folder in enumerate(subfolders):
    ...
    turbidity_path = os.path.join(folder, f'{cite_number}.turbidity.Raw_Data.csv')
    salinity_path = os.path.join(folder, f'{cite_number}.salinityaselectricalconductivityec.Raw_Data.csv')
    ph_path = os.path.join(folder, f'{cite_number}.ph.Raw_Data.csv')
    ...
    turbidity_df = pd.read_csv(turbidity_path, parse_dates=['Date Time'])
    salinity_df = pd.read_csv(salinity_path, parse_dates=['Date Time'])
    ph_df = pd.read_csv(ph_path, parse_dates=['Date Time'])
    ...
    turbidity_df.set_index('Date Time', inplace=True)
    salinity_df.set_index('Date Time', inplace=True)
    ph_df.set_index('Date Time', inplace=True)
    ...
    merged_df = merged_df.join(turbidity_df, rsuffix='_turbidity', how='inner')
    merged_df = merged_df.join(salinity_df, rsuffix='_salinity', how='inner')
    merged_df = merged_df.join(ph_df, rsuffix='_ph', how='inner')
